        ...
        {confirmDm ? (
          <div className="flex flex-col items-center py-6 text-center">
            <div className="relative mb-6">
              {confirmDm.profile || confirmDm.avatar_url || confirmDm.profilePic ? (
                <img src={confirmDm.profile || confirmDm.avatar_url || confirmDm.profilePic} className="h-24 w-24 rounded-full object-cover border-4 border-primary/20 shadow-xl" alt="" />
              ) : (
                <div className="h-24 w-24 rounded-full bg-primary/10 flex items-center justify-center text-3xl font-bold text-primary shadow-inner">
                  {(confirmDm.name || confirmDm.display_name || confirmDm.username || "?").slice(0, 1).toUpperCase()}
                </div>
              )}
            </div>
            <DialogHeader className="mb-6">
              ...
              <DialogDescription className="text-base mt-2">
                Would you like to start a chat with <span className="font-semibold text-foreground">{confirmDm.name || confirmDm.display_name}</span>?
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="mt-4 flex flex-wrap gap-1.5">
                {selected.map((p) => (
                  <button key={p.id || p._id} onClick={() => toggle(p)}
                    ...
                    {p.name || p.display_name || p.username || p.email}
            ...
            <div className="mt-4 max-h-[300px] overflow-y-auto rounded-xl border border-border bg-muted/10">
              ...
              {searchResults.map((p) => {
                ...
                return (
                  ...
                  >
                    {p.profile || p.avatar_url || p.profilePic
                      ? <img src={p.profile || p.avatar_url || p.profilePic} className="h-10 w-10 rounded-full object-cover border border-border" alt="" />
                      : <div className="grid h-10 w-10 place-items-center rounded-full bg-primary/10 text-xs font-bold text-primary border border-primary/20">
                        {(p.name || p.display_name || p.username || p.email || "?").slice(0, 2).toUpperCase()}
                      </div>}
                    <div className="min-w-0 flex-1">
                      <div className="truncate text-sm font-semibold">{p.name || p.display_name || p.username || "Unnamed"}</div>
